...
contract MockParaSwapAugustus is IParaSwapAugustus {
    ...
    function expectBuy(address fromToken, address toToken, uint256 fromAmount, uint256 toAmountMin, uint256 toAmountMax)
        external
    {
        _expectingSwap = true;
        _expectedFromToken = fromToken;
    ...
    function swap(address fromToken, address toToken, uint256 fromAmount, uint256 toAmount)
        external
        returns (uint256)
    {
        ...
        require(
            fromAmount >= _expectedFromAmountMin && fromAmount <= _expectedFromAmountMax, "From amount out of range"
        );
