        ...
        returns (AggregatedReserveData[] memory, BaseCurrencyInfo memory)
    {
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            reserveData.availableLiquidity =
                IERC20Detailed(reserveData.underlyingAsset).balanceOf(reserveData.aTokenAddress);
            (
      ...
      user != address(0) ? reserves.length : 0
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            if (userConfig.isBorrowing(i)) {
                ...
                userReservesData[i].principalStableDebt =
                    IStableDebtToken(baseData.stableDebtTokenAddress).principalBalanceOf(user);
                if (userReservesData[i].principalStableDebt != 0) {
                    userReservesData[i].stableBorrowRate =
                        IStableDebtToken(baseData.stableDebtTokenAddress).getUserStableRate(user);
                    userReservesData[i].stableBorrowLastUpdateTimestamp =
                        IStableDebtToken(baseData.stableDebtTokenAddress).getUserLastUpdated(user);
                }
            }
        }
